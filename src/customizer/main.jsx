...
import { workTime, breakTime, variables } from "../index.js"
import { isMovable } from "../movable/main.jsx"
import { isEditable } from "../editable/main.jsx"
import Input from "./input.jsx"
import Accordion from "../accordion/main.jsx"
import FontButtons from "./fontButton.jsx"
import "./main.css"
...
export default function Customizer() {
    ...
    const[vars, setVars] = useAtom(variables)
    const[bgColor, setBgColor] = useState(vars["--bg-color"] ?? computedStyle.getPropertyValue("--bg-color"))
    const[mColor, setMColor] = useState(vars["--main-color"] ?? computedStyle.getPropertyValue("--main-color"))
    const[sColor, setSColor] = useState(vars["--sec-color"] ?? computedStyle.getPropertyValue("--sec-color"))
    useEffect(() => {
        ...
        <>
         <div className="settings-container" style={containerStyle}>
            ...
            <Accordion name="Color">
                <Input name={`Set Theme Color: `} type="color" initial={ rgbToHex(mColor) } onChange={ (val) => {
                    const realVal = hexToRgb(val)
                    setVars( (prev) => {prev["--main-color"] = realVal; return prev} )
                    setMColor( realVal )
                }  }/>
                <Input name={`Set Theme Background Color: `} type="color" initial={ rgbToHex(bgColor) } onChange={ (val) => {
                    const realVal = hexToRgb(val)
                    setVars( (prev) => {prev["--bg-color"]  = realVal; return prev} )
                    setBgColor( realVal )
                }  }/>
                <Input name={`Set Theme Secondary Color: `} type="color" initial={ rgbToHex(sColor) } onChange={ (val) => {
                    const realVal = hexToRgb(val)
                    setVars( (prev) => {prev["--sec-color"]  = realVal; return prev} )
                    setSColor( realVal )
                }  }/>
                <br></br>
                <Input name={`Background Opacity: `} type="range" min="0" max="1" step="0.01" onChange={(val) => { 
                        setBgColor( (prev) => {
                            ...
                            setVars( (prev) => {prev["--bg-color"]  = cool; return prev})
                            return cool
                        } ) 
                }} />
                <Input name={`Secondary Opacity: `} type="range" min="0" max="1" step="0.01" onChange={(val) => { 
                        setSColor( (prev) => {
                            const result = getIndividualRGB(prev)
                            const cool = `rgba(${result[0]}, ${result[1]}, ${result[2]}, ${val})`
                            setVars( (prev) => {prev["--sec-color"]  = cool; return prev})
                            return cool
                        } ) 
                }} />
            </Accordion>
            <Accordion name="Main Font">
                <FontButtons toSet="--basic-font"/>
            </Accordion>
            <Accordion name="Title Font">
                <FontButtons toSet="--title-font"/>
            </Accordion>
